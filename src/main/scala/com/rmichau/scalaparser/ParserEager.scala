...
object Parser {
  // needs to take expecting by name to handle recursive parsers
  def apply[E, R](f: (String, Int) => ParseResult[E, R], expecting_ : => String): Parser[E, R] = new Parser[E, R] {
    override def apply(str: String, idx: Int): ParseResult[E, R] = f(str, idx)

    override def expecting: String = expecting_
  }
}

abstract class Parser[E, R] {
  def apply(str: String, idx: Int = 0): ParseResult[E, R]
...
object Parsers {
  ...
  def BEGIN: ParserStE[Unit] = {
    Parser(
      (tokens: String, idx: Int) => {
        if (idx == 0) Validated.valid((idx, ()))
        else Validated.invalidNec(s"expected BEGIN, got ${tokens(idx)}")
      },
      expecting_ = "BEGIN"
    )
  }
...
object Combinators {
  ...
  def REPEAT_SEPARATOR[T, T2](
    ...
    Parser((tokens, idx) => loop(tokens, idx, Nil, minIteration), expecting_ = s"loop of ${parser.expecting}")
  }
  ...
  def REPEAT[T](parser: => ParserStE[T], minIteration: Int = 0): ParserStE[Seq[T]] = {
    Parser(
      ...
      expecting_ = s"Loop of ${parser.expecting}"
    )
  }
  ...
  def ANY_OF[T](parsers: => Seq[ParserStE[? <: T]]): ParserStE[T] = {
    ...
    Parser(
      ...
      expecting_ = s"One of ${ps.map(_.expecting).mkString("/")}"
    )
  }
  ...
  def OPTION[T](parser: => ParserStE[T]): ParserStE[Option[T]] = {
    Parser(
      ...
      expecting_ = s"Optional ${parser.expecting}"
    )
  }
